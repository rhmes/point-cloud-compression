...
ae.load_state_dict(torch.load(NET_PATH, map_location=args.device))
ae.eval()

# PROB MUST RUNNING ON THE GPU (don't know why...)
prob = AE.ConditionalProbabilityModel(args.L, args.d).to(args.device)
prob.load_state_dict(torch.load(PROB_PATH, map_location=args.device))
prob.eval()
...
for i in tqdm(range(len(filenames))):
    ...
    # GET pmf
    pmf = prob(rec_sampled_xyz.to(args.device))
    ...
    pn_kit.save_point_cloud(pc[0].detach().cpu().numpy(), filenames[i] , path=args.decompressed_path)
