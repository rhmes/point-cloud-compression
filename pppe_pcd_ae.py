...
class ConditionalProbabilityModel(nn.Module):
    """
    Conditional Probability Model for Point Cloud Compression.
    Uses PointNet2Encoder features to condition entropy parameters.
    Input: latent codes y (B, C, N)
    ...
    def __init__(self, hidden_channels=128, latent_dim=256, npoints=8192):
        ...
        feature_dim = 128  # per-point features from encoder

        # combine latent+encoder features
        self.model_mlp = nn.Sequential(
            nn.Conv1d(3 + feature_dim , hidden_channels, 1),
            nn.ReLU(),
...
class RateDistortionLoss(nn.Module):
    def __init__(self, loss_type="hybrid", alpha=0.7, eps=1e-9):
        """
        Args:
            loss_type : "chamfer", "l2", or "hybrid"
            alpha     : weight for Chamfer in hybrid mode
        """
    ...
    def forward(self, pc_pred, pc_target, fbpp, λ=1.0):
        ...
        if self.loss_type == "chamfer":
            ...
            dist = dist / pc_pred.shape[1]  # normalize
